...
| Named employers appear only in the one Selected work entry | case-insensitive grep for `Tyler|Cloud303` across `out/` | pass, 1 visible-HTML occurrence in `out/index.html` plus its RSC payload copies | A second, less hedged employer mention elsewhere |
